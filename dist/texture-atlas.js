...
class TextureAtlas {
     ...
     */
    constructor({ texture, tileWidth, tileHeight, cols, rows, count, offset = 0, trim = 0, scaleMode = 'nearest' }) {
        /**
         * texture slices
         */
        this.slices = [];
    ...
    loadSlice(slice) {
        ...
        const frame = new PIXI.Rectangle(this.trim + x, this.trim + y, this.tileWidth - this.trim * 2, this.tileHeight - this.trim * 2);
        const texture = ('source' in this.texture
            ? new PIXI.Texture({
     ...
     */
    preload(count = Math.floor((this.width / this.tileWidth) * (this.height / this.tileHeight))) {
